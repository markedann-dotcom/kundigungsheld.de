  ...
  return (
    <section id="abo-rechner" className="py-20 bg-muted/20 border-y border-border/30">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="mx-auto max-w-4xl grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
          ...
          <div className="space-y-3">
            ...
            {abos.map((abo) => (
              <div
                key={abo.id}
                className="rounded-xl border border-border/60 bg-background px-3 py-2.5 group"
              >
                {/* Row 1: Name + Delete */}
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <button
                    ...
                    className="shrink-0 p-1 rounded-md text-muted-foreground/30 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-colors"
                  >
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                </div>

                {/* Row 2: Amount + Interval + Yearly equiv */}
                <div className="flex items-center gap-2 flex-wrap">
                  {/* Amount */}
                  <div className="flex items-center gap-1 rounded-lg border border-border/60 px-2.5 py-1.5">
                    <input
                      type="number"
                      value={abo.amount}
                      ...
                      min="0"
                      step="0.01"
                      className="w-16 bg-transparent text-sm text-foreground text-right outline-none placeholder:text-muted-foreground/40"
                    />
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <div className="relative">
                ...
                {showPresets && (
                  <div className="absolute top-full left-0 mt-2 z-10 w-56 max-w-[calc(100vw-2rem)] rounded-xl border border-border/60 bg-background shadow-lg overflow-hidden">
                    {PRESETS.map((p) => (
                      <button
                        key={p.name}
                        onClick={() => addPreset(p)}
